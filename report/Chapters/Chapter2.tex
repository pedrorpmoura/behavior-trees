...
\paragraph{Sequence}{
    Um nodo \textit{Sequence} visita (envia \textit{ticks}) a todos os filhos por ordem, começando pelo primeiro, e se este retornar \textit{Success}, chama o segundo, e por aí em diante.
...
\paragraph{Decorator}
    Um nodo do tipo \textit{Decorator}, é um nodo com um unico filho, este manipula o estado de retorno consoante as regras definidas pelo utilizador.
    Um exemplo da utilização deste tipo de nodos seria usar um nodo \textit{Decorator}, dizendo que o seu filho só pode correr n vezes, caso não corra retorna que \textit{Failure}, caso contrário retorna \textit{Sucess}.
    Outro exemplo seria dizer que o filho só poderia correr durante x segundos, se após esses x segundos o filho ainda esteja a correr, retorna \textit{Failure}, caso contrário retorna \textit{Sucess}.
