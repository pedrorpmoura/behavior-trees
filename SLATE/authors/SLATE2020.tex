...
Each node returns one of the following three states to its parent: \textbf{SUCCESS}, \textbf{FAILURE} or \textbf{RUNNING}.
...
In case a child fails, the node returns FAILURE.
If all children succeed, it returns SUCCESS.
And if a child returns RUNNING, the node also returns RUNNING.
